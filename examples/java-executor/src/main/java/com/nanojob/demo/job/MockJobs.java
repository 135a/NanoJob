...
public class MockJobs {
    ...
    /**
     * 示例 1：模拟业务型任务 (如信贷计息)。
     * 新架构是【单目标路由】——引擎按 appName 从注册表挑一台执行器派发 /run，
     * 不再有分片广播；任务跑完由 xxl-job-core 自动 POST /api/callback 回填执行日志。
     */
    @XxlJob("loanInterestJobHandler")
    public void loanInterestJobHandler() throws Exception {
        System.out.println("\n🚀 [信贷计息任务] 收到指令！开始处理本机负责的数据...");

        // 模拟执行耗时业务
        for (int i = 0; i < 5; i++) {
            System.out.println("   -> ⚙️ 正在计算第 " + (i + 1) + " 批利息...");
            TimeUnit.MILLISECONDS.sleep(600);
        }

        System.out.println("✅ [信贷计息任务] 数据处理完毕！等待 Go 引擎下一次召唤。");
