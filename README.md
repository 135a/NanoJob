...
3. **高可用主备架构与脑裂防御 (Split-Brain Defense)**：基于 etcd `concurrency.NewElection` 实现了强一致性的分布式抢锁机制。在 K8s 多副本部署时，仅有一台机器可当选为 Leader 执行时间轮转动，其余节点挂起作为 Standby。搭配完善的阻塞隔离机制，彻底杜绝了网络抖动下多节点同时派发任务导致的灾难性重复执行。
4. **无状态动态注册中心 (Stateless Registry)**：彻底摒弃了传统的内存 `sync.Map` 状态孤岛。接入端的心跳数据直接绑定 etcd 租约 (Lease)。任何一台引擎节点宕机，etcd 自动回收过期记录，Leader 派发任务时实时查询全网一致性视图，确保分片路由绝对精准。
5. **Misfire 漏发补偿机制**：在发生全网大停电或集群崩溃等极端宕机场景下，核心业务绝不丢失。引擎恢复重启并当选 Leader 的瞬间，会自动触发“秋后算账”逻辑，精准识别宕机期间错漏的任务，并强制执行 `FIRE_ONCE_NOW` 立即补偿，随后平滑回归标准时间轮，完美保障资金级任务的绝对闭环。
